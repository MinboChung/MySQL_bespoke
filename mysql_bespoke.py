...
class MySQLConnector(ABC):
    ...
    def delete_database(self, db_name:str=None):
        ...
        db_list = [x for x in my_cursor if x[0] == db_name]
        if len(db_list) == 0:
            print(f"{db_name} is removed")
        else:
            print(f"{db_name} is not removed")


if __name__ == "__main__":
    my_db_connection = MySQLConnector()
    my_db_connection.connect()
    # my_db_connection.delete_database(db_name="heavenms")
    # my_db_connection.create_database(db_name="first_db")
